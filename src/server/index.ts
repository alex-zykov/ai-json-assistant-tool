...
export class AIJSONServer {
    ...
    private buildConfig(userConfig: Partial<ServerConfig>): ServerConfig {
        return {
            ...
            schemaPath: userConfig.schemaPath || process.env.SCHEMA_PATH || path.join(process.cwd(), 'schema.json'),
            promptPath: userConfig.promptPath || process.env.PROMPT_PATH || path.join(process.cwd(), 'prompt.json'),
    ...
    private async handleGenerate(req: express.Request, res: express.Response): Promise<void> {
        ...
        if (!this.factory || !this.defaultSchema) {
    ...
    public async initialize(): Promise<void> {
        try {
            ...
            console.log(chalk.cyan('Schema properties:'),
                chalk.green(Object.keys(schemaData.properties).join(', ')));
