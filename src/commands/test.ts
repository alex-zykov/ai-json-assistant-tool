...
Environment Variables:
  You can set defaults using environment variables:
    ...
    PROMPT_PATH=./prompt.json (optional)
...
Examples:
  ...
  Use custom schema and override with prompt file:
    $ ai-json test -s ./custom-schema.json -p ./system-prompt.json test.jsonl
        ...
        .action(async (jsonlPath: string, options: TestOptions) => {
            try {
                ...
                if (!schemaData['type'] || schemaData['type'] !== 'object' || !schemaData['properties']) {
                    throw new Error(`Invalid schema format: ${options.schema}. Schema must be an object type.`);
                } else {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    try {
                        ...
                        // Parse expected response as JSON to validate it
                        let parsedExpected;
                        try {
                ...
                if (error instanceof Error) {
                    if (error.message.includes('ENOENT')) {
                        ...
                        } else if (error.message.includes(options.schema)) {
